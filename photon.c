...
char infos [622] = "{\"MotorState\" : 0, \"Direction\" : 120,\"Distance\" : 0,\"Temperature\" : 0,\"Brightness\"  : 0,\"Noise\" : 0,\"AlertStatus\" : 0,\"PictureNumber\" : 0}";
...
int RIOTread(uint8_t *msg, uint8_t *msglen)
{
	...
	while ((istop != 2) && (i < MSG_SIZE_MAX))
	{
		...
		if (ibuf == -1) return -10; // serial buffer empty, should happen only in case of timeout
...
int robotInfos() {
    ...
    Serial.println("Begin robotInfos");
    ...
    Serial.print("value[9]: "); Serial.print((int)value[9]);               
    Serial.print("value[10]: "); Serial.print((int)value[10]);               
    
    // encode in JSON
    sprintf(infos,"{\"MotorState\": %lu, \"Direction\": %lu, \"Distance\": %lu, \"Temperature\": %lu, \"Brightness\": %lu, \"Noise\": %lu, \"AlertStatus\": %lu, \"PictureNumber\": %lu}",value[1],value[4],value[5],value[6],value[7],value[8],value[9],value[10]);
...
int robotCmd(String command) {
  ...
  {   
       ...
       if (freqInfos == 0) freqInfos = 60*1000; // get infos as soon a command is send
